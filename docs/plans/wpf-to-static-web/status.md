...
- 2026-07-11 14:31 +09:00: 公開サイトの実Edge・実マイク試験で初回権限前の空`deviceId`による失敗を再現。`getUserMedia({audio:true})`で権限取得後、既定マイク48kHzの取得、Spectorのライブ値表示、停止・解放を確認。Task 07として初回権限取得順序の修正を開始。
- 2026-07-11 14:38 +09:00: Task 07実装完了。`BrowserAudioEngine.initialize()`で権限用ストリームを先に取得・即停止し、権限後のデバイス一覧だけを公開するよう修正。空IDから有効IDへ変わる回帰、権限拒否、リソース解放を追加し、親再実行のformat/lint/typecheck、Vitest 136件、Playwright 5件、production build、diff-checkは成功。修正版localhostの実Edge再試験はEdge側リモートデバッグが無効のため未実施。
